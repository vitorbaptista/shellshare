...
if (!args.includes('--follow')) {
  ...
  // Exits once stdout has flushed rather than waiting for the socket to let
  // the loop drain: a close handshake the peer never answers would otherwise
  // hold the process open. The flush callback is what keeps this from
  // truncating its own output.
  const stop = (msg) => {
    clearTimeout(ending);
    if (msg) die(msg);
    try { ws.close(); } catch {}
