...
- `termtheme.rs`: asks the terminal for its own colors (OSC 11 background, OSC 10 foreground, OSC 4 for each of the 16 ANSI slots) so a broadcast can be shown in the broadcaster's real theme rather than a preset. `--theme` defaults to `auto`, which means this detection; an explicit `--theme` is never second-guessed. A detected theme has no name, so it rides the `size` message as `colors` by value (`{foreground, background, palette}`, the `themes.json` shape) next to where a name would go, and the viewer prefers it (`public/javascript/room.js`). Every failure path - Windows/ConPTY, no controlling `/dev/tty`, a terminal that ignores the queries, a partial palette - falls back to `tango`, the name the CLI defaulted to before detection existed, so nothing regresses where detection is impossible. Two measured facts it encodes: replies terminate with `ST` *or* `BEL` (the same terminal switches to `BEL` behind tmux, so both must be accepted), and tmux forwards the outer terminal's real colors rather than its own. Queries `/dev/tty` rather than stdin/stdout so detection survives `dmesg | shellshare`
...
UTF-8 decode (the viewer script is `public/javascript/room.js`;
...
- **Lockstep contracts** that span files and break silently if drifted - the wire protocol (`protocol.rs` ↔ `public/javascript/room.js` ↔ `conftest.py`), the encryption record format, the `size` control message (theme by name, detected `colors` by value, `encrypted` flag, cols/rows omission), and the `--json` event contract (`sharing`/`end`, mirrored in `AGENTS.md` + `templates/llms.txt`). Test each contract once, at the layer that owns it.
