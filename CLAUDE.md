...
- **Lockstep contracts** that span files and break silently if drifted - the wire protocol (`protocol.rs` ↔ `public/javascript/room.js` ↔ `conftest.py`), the encryption record format, the `size` control message (theme, `encrypted` flag, cols/rows omission), and the `--json` event contract (`sharing`/`end`, mirrored in `AGENTS.md` + `templates/llms.txt`). Test each contract once, at the layer that owns it.
