...
mod tests {
    ...
    #[test]
    fn test_platform_from_query_param() {
        assert_eq!(
            Platform::from_query_param("linux"),
        ...
        assert_eq!(
            Platform::from_query_param("windows"),
            Some(Platform::WindowsX86_64)
        );
        assert_eq!(
            Platform::from_query_param("win"),
            Some(Platform::WindowsX86_64)
        );
        assert_eq!(
            Platform::from_query_param("mac"),
            Some(Platform::MacosX86_64)
        );
        assert_eq!(
            Platform::from_query_param("macos"),
            Some(Platform::MacosX86_64)
        );
        assert_eq!(
            Platform::from_query_param("mac-intel"),
            Some(Platform::MacosX86_64)
        );
        assert_eq!(
            Platform::from_query_param("mac-arm"),
            Some(Platform::MacosAarch64)
        );
        assert_eq!(Platform::from_query_param("unknown"), None);
    }
    ...
    fn test_platform_from_user_agent() {
        ...
        // macOS ARM
        assert_eq!(
            Platform::from_user_agent("curl/8.0.1 (aarch64-apple-darwin)"),
            Some(Platform::MacosAarch64)
        );
        assert_eq!(
            Platform::from_user_agent("Mozilla/5.0 (Macintosh; arm64)"),
            Some(Platform::MacosAarch64)
        );

        // macOS Intel (default for mac without arch)
        assert_eq!(
            Platform::from_user_agent("Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)"),
            Some(Platform::MacosX86_64)
        );
        assert_eq!(
            Platform::from_user_agent("curl/8.0.1 (x86_64-apple-darwin)"),
            Some(Platform::MacosX86_64)
        );

        // Linux
        assert_eq!(
            Platform::from_user_agent("curl/7.81.0 (x86_64-pc-linux-gnu)"),
            Some(Platform::LinuxX86_64)
        );
        assert_eq!(
        ...
        assert_eq!(Platform::from_user_agent("curl/8.0.1"), None);
    }

    #[test]
    fn test_platform_filename() {
        assert_eq!(Platform::LinuxX86_64.filename(), "shellshare");
        assert_eq!(Platform::WindowsX86_64.filename(), "shellshare.exe");
        assert_eq!(Platform::MacosX86_64.filename(), "shellshare");
        assert_eq!(Platform::MacosAarch64.filename(), "shellshare");
    }
}
